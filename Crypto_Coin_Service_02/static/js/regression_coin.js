  ...
  anal_btn.addEventListener("click", async function () {
    ...
    if (padding) {
      ...
      <p style="font-size:1.5rem;margin:1rem">최고가오차율(${
        parseInt(info_data["err_rate"]["high"] * 10000) / 100
      }%)
      현재가오차율(${parseInt(info_data["err_rate"]["cur"] * 10000)/100}%)
      최저가오차율(${parseInt(info_data["err_rate"]["low"] * 10000)/100}%)
      </p>
      <div>
        <h2>훈련 결과 그래프</h2>
      ...
      for (let data of info_data["ypred"]) {
        ...
        inHtml += ` <div style='padding:0.5rem;border:2px solid darkgray;
                                          margin-bottom:1rem'>
            <p style='padding:0.5rem;background:green'>
            ${today_str} </p>
